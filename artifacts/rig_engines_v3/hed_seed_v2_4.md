...
We killed the "AI literacy program" approach because it is consultant theater (no operating system, no compounding loop, no defense moat leverage). We abandoned the "Forge as standalone SaaS" approach because it dropped the hardware-firmware coupling that creates the structural attach economics. We deprecated the "Bosch-style commercial telematics" path because Bosch's 36-month Trackunit head start is structurally insurmountable. We dropped the chatbot bolt-on pattern explicitly. Each kill has its reason; we prune the weak, stale, low-signal options at every phase.
